# Home Lab 
Setting up a Windows + Linux + SIEM lab for learning defensive security

# Next Steps

Stage 1
- [x] Add Windows VM 
- [x] Add Kali VM 
- [x] Document Nmap scans
- [x] Create an Ubuntu Wazuh SIEM Server

Stage 2
- [x] Fine-tuning Wazuh detection rules, (Side Goal: See Wazuh ping a high-risk alert)
- [x] Set up vulnerable target VM (Metasploitable2)
- [x] Simulate an attack from Kali against the vulnerable VM
- [ ] (PARTIAL SUCCESS) Capture and document the full detection trail in Wazuh (WIP- Metasploitable Legacy Software)
- [ ] Update Kali agent to match manager version (v4.9.2)

Stage 3
- [ ] Deploy a Velociraptor server + agents for DFIR/Threat hunting 
